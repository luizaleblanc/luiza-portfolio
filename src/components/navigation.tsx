...
const MobileLink: React.FC<MobileLinkProps> = ({ href, className, children, onClick }) => (
  <motion.a
    href={href}
    onClick={onClick}
    ...
    className={clsx(
      "w-fit flex items-center justify-center gap-3 text-lg sm:text-2xl text-zinc-100 focus-visible:outline-2 outline-offset-4 outline-zinc-100 rounded-lg cursor-pointer",
      className
    )}
  >
    {children}
...
export default function Navigation() {
  ...
  if (!isMounted) return null; // Evita layout shift / hydration mismatch

  const desktopLinkClasses =
    "w-fit text-base sm:text-2xl px-5 py-2 rounded-full transition-all duration-300 ease-in-out whitespace-nowrap text-zinc-100 hover:text-white bg-transparent hover:bg-zinc-100/20 focus-visible:outline-2 outline-offset-2 outline-zinc-100 rounded-lg";

  return (
    <nav className="flex items-center justify-center gap-12" aria-label="Main Navigation">
      <AnimatePresence>
        {isOpen && (
          <>
            <motion.div
            ...
            <motion.div
              ...
              animate={{ filter: "blur(0px)", opacity: 1 }}
              exit={{ filter: "blur(20px)", opacity: 0 }}
              className="lg:hidden fixed inset-0 z-40 w-full h-full flex flex-col gap-4 bg-zinc-900/40 backdrop-blur-md"
            >
              <div className="fixed bottom-20 sm:bottom-32 left-4 sm:left-8 flex flex-col items-start justify-center gap-4">
                {NAV_ITEMS.map((item) => (
                  <MobileLink key={item.href} href={item.href} onClick={() => setIsOpen(false)}>
                    {item.icon}
                    {item.label[locale]}
                  </MobileLink>
                ))}
                ...
                <motion.button
                  ...
                  aria-label={UI_DICTIONARY.close[locale]}
                  initial={{ transform: "translateY(100px)", filter: "blur(20px)", opacity: 0 }}
                  transition={{ type: "spring", stiffness: 25, damping: 2, mass: 0.1 }}
                  animate={{ transform: "translateY(0px)", filter: "blur(0px)", opacity: 1 }}
                  exit={{ transform: "translateY(100px)", filter: "blur(20px)", opacity: 0 }}
                  className="w-fit flex items-center justify-center gap-3 text-lg sm:text-2xl text-zinc-100 cursor-pointer focus-visible:outline-2 outline-offset-4 outline-zinc-100 rounded-lg mt-4"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" aria-hidden="true">
      ...
      <button
        ...
        aria-label={UI_DICTIONARY.menu[locale]}
        onClick={() => setIsOpen((prev) => !prev)}
        className="w-6 sm:w-8 h-fit text-white cursor-pointer hover:outline-2 focus-visible:outline-2 outline-offset-4 outline-zinc-100/20 focus-visible:outline-white rounded-lg z-50 relative"
      >
        <svg
          ...
          viewBox="0 0 24 24"
          aria-hidden="true"
          className={clsx("transition-transform duration-300 ease-in-out", isOpen ? "rotate-90" : "rotate-0")}
        >
